...
Third, bug repair has structured failure modes --- off-by-one boundary shifts, inverted conditions, wrong variable references, missing guard clauses --- that reward specialization in a measurable and reproducible way.
...
\textbf{Phase~3 (Final Evaluation)} scores the final champion --- and all ablation variants --- on the held-out test set that was never touched during search.
...
Tasks that did not satisfy this invariant --- bugs too subtle for the provided tests to catch --- were redesigned with targeted test cases that expose the fault.
